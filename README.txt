...
例
 $ python ics2gacsv.py 202509 icalendar.ics kiroku202509.csv
...
*その他プログラムの仕様やバグなど。
icsも細かい方言あるため、うまく変換できるかわからない。

icsファイルの時刻情報にTimeZoneがない場合は、TimeZoneなしとして処理す
る。出力のCSVもTimeZoneは想定してない。TimeZoneありのicsとTimeZoneなし
のicsを混ぜてはいけない。混ぜるとプログラムは停止する。

Web版のOutlookのicsはTimeZone情報あり。GaroonのicsはTimeZone情報なし。

バグ1: 2099年までしか動きません。2100年になったらバグります。要: 正規
表現を修正する。

バグ2: TimeZone情報を全く考えてないです。9時間ずれるかもしれない。
ICSのRRULEのUNITLのバグ処理の都合で、コードがJSTを想定している。
  →bug_fix_rrule_aux()
