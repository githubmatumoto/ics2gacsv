...
TIMERANGE=202512
#sjis
python3 ics2gacsv.py       $TIMERANGE calendar.ics  gen-debug-sjis.csv
python3 ics2gacsv.py -m    $TIMERANGE calendar.ics  gen-debug-sjis-m.csv
python3 ics2gacsv.py       $TIMERANGE calendar.ics  stdout > gen-debug-stdout.csv
#utf-8
python3 ics2gacsv.py -u    $TIMERANGE calendar.ics  gen-debug-u.csv
...
python3 ics2gacsv.py -u -m $TIMERANGE calendar.ics  gen-debug-m.csv
#
python3 ics2gacsv.py -u -s $TIMERANGE calendar.ics  gen-debug-s.csv
python3 ics2gacsv.py -u -t $TIMERANGE calendar.ics  gen-debug-t.csv
python3 ics2gacsv.py -u -o $TIMERANGE calendar.ics  gen-debug-o.csv
python3 ics2gacsv.py -u -g $TIMERANGE calendar.ics  gen-debug-g.csv
python3 ics2gacsv.py -u -d $TIMERANGE calendar.ics  gen-debug-d.csv
python3 ics2gacsv.py -u -p $TIMERANGE calendar.ics  gen-debug-p.csv
python3 ics2gacsv.py -u -r $TIMERANGE calendar.ics  gen-debug-r.csv
python3 ics2gacsv.py -u -w $TIMERANGE calendar.ics  gen-debug-w.csv
python3 ics2gacsv.py -u -z $TIMERANGE calendar.ics  gen-debug-z.csv

#
echo "TimeZoneとしてJSTとESTを明示指定。"
echo "明示指定のTimeZoneはTimeZoneが不明な文脈でのみ利用される"
python3 ics2gacsv.py -u  -T"Asia/Tokyo"  $TIMERANGE calendar.ics  gen-debug-jst.csv
echo "以下は(おそらく)ICSが想定しているTimeZoneと異なる値を指定"
python3 ics2gacsv.py -u  -T"US/Eastern"  $TIMERANGE calendar.ics  gen-debug-est.csv

echo "以下はRECURRENCE-IDの処理をしないため多数警告でる場合あるが、それで正常"
python3 ics2gacsv.py -u -x $TIMERANGE calendar.ics  gen-debug-x.csv

echo "以下はUNTILの処理をしないため例外をはく場合があるが、それで正常"
python3 ics2gacsv.py -u -b $TIMERANGE calendar.ics  gen-debug-b.csv
...
echo "拡張業務番号のあるなし"
echo diff gen-debug-u.csv gen-debug-z.csv
